...
- [ ] Create scripts/animation-critic.ts (can be a standalone Node script)
- [ ] Accepts: directory of frame PNGs + scene description string
- [ ] Selects key frames (every 30th, or first/middle/last)
- [ ] Sends to Claude vision API (use Anthropic SDK, API key from env ANTHROPIC_API_KEY)
- [ ] Prompt template: "This is a stick figure animation of [description]. These frames show the sequence. Rate the animation quality 1-10. List specific issues with frame numbers."
- [ ] Structured output: { score: number, issues: [{ frame: number, description: string }], summary: string }
- [ ] Temperature=0 for reproducibility
- [ ] Test with The Kick scene captures from Phase B
